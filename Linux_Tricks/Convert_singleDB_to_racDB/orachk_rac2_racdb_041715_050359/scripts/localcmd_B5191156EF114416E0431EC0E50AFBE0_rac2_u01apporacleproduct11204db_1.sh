...
. /home/oracle/.orachk/set_orcl_env_patches.sh
opatch_return_code=$($ORACLE_HOME/OPatch/opatch lsinventory -oh $ORACLE_HOME -local >/dev/null 2>&1;echo $?);if [ $opatch_return_code -eq 0 ]; then opatch_output=$($ORACLE_HOME/OPatch/opatch lsinventory -oh $ORACLE_HOME -local -bugs_fixed);opatch_output_main=$($ORACLE_HOME/OPatch/opatch lsinventory -oh $ORACLE_HOME -local|grep -wi "patch description");else opatch_output=$(cat $ORACLE_HOME/inventory/ContentsXML/comps.xml);fi; echo -e "$opatch_output_main\n\nexachk_patchlist_for_version_matrix\n\n$opatch_output"
echo -e "$opatch_output" >>/home/oracle/orachk/orachk_041715_050359/o_rdbms_patches_u01apporacleproduct11204db_1_rac2_report.out
