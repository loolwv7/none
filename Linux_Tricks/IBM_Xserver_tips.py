= IBM About =
== Log collection ==
https://www-947.ibm.com/support/entry/portal/docdisplay?lndocid=tool-center
https://www-947.ibm.com/support/entry/portal/docdisplay?lndocid=SERV-DSA
http://publib.boulder.ibm.com/infocenter/systemx/documentation/index.jsp?topic=/com.ibm.sysx.7945.doc/c_diagnostictools.html
...
Specify -v to convert the data to HTML format.
Specify -text to convert the data to ASCII text format.
Specify -ux to compare firmware levels to the latest available updates & System
Packs.
Specify -t to upload the DSA output directly to the IBM DSA FTP Database.
Specify -d to set the output_directory (for example:/tmp/DSA or c:\temp\DSAforWindows)
Specify -c to create the output directory if it does not already exist.

 * This will let you take an existing DSA file and reparse the data on another system.
{{{
dsa.exe -b -v -i dsa.xml.gz(output xml)
}}}
